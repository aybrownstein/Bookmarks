...
const MyBookmarks = () => {
    ...
    const onUpdateClick = async (title, bookmarkId) => {
        await axios.post('/api/home/updatetitle', {title, bookmarkId});
        await getBookmarks();
        setEditIds(editIds.filter(id => id !== bookmarkId));
    }

    const onDeleteClick = async bookmarkId => {
        await axios.post('/api/home/delete', {bookmarkId});
        await getBookmarks();
    }
